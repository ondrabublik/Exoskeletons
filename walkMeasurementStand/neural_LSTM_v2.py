...
# ====== Načtení dat ze všech souborů v adresáři ======
data_files = glob.glob(os.path.join(DATA_DIR, "*.TXT"))
print(f"Načítám data z {len(data_files)} souborů...")

all_data = []
for file_path in data_files:
    print(f"  Načítám: {os.path.basename(file_path)}")
    file_data = np.loadtxt(file_path, delimiter=",")
    all_data.append(file_data)

# Zkombinování všech dat do jednoho pole
data = np.vstack(all_data)
print(f"Celkem načteno {len(data)} vzorků")
...
print("\nModel uložen jako lstm_model_v2.h5")
